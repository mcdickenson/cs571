...
\paragraph{G} Bivariate plots of $X_1$ and $X_2$ with $Y$ are given in Figures \ref{p1x1} and \ref{p1x2} below, along with regression lines for the three methods used above. Note that the lines for the normal equation method and the stochastic gradient descent (SGD) method overlap in both cases. (The intercept and $\hat{\beta}_i$ are used in the plot for $X_i$, while $\hat{\beta}_j, j\neq i$ is ignored.)

\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.5]{p1x1.pdf}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.5]{p1x2.pdf}
\caption{$X_2$ and $Y$ with Regression Lines}
\label{p1x2}
\end{center}
\end{figure}
...
\paragraph{G} Figures \ref{p2x1} and \ref{p2x2} below plot $X_1$ and $X_2$ with $Z$ and the respective regression line. Because $X_1$ and $Z$ are binary variables, a small random jitter is added to make the points more apparent. In Figure \ref{p2x1} the intercept is set to the mean of $Z$ and in Figure \ref{p2x2} the intercept is set to $\hat{\beta}_2$; in each case this is needed to make the line visible on the plot. 
